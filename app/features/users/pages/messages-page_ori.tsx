...
export const loader = async ({ request }: Route.LoaderArgs) => {
  console.log("loader 시작");
  const { client } = makeSSRClient(request);
  console.log("makeSSRClient 완료");
  const { data, error } = await client.auth.getUser();
  console.log("getUser 결과:", data, error);
  const user = data?.user;
  if (!user) {
    console.log("user 없음, redirect");
    return redirect('/auth/login');
  }
  const conversations = await getConversations(client, { profileId: user.id });
  console.log("conversations 결과:", conversations);
