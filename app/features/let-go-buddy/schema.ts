...
import { DECLUTTER_SITUATIONS, EMOTIONAL_ATTACHMENT_LEVELS, ENVIRONMENTAL_IMPACT_LEVELS, RECOMMENDATION_ACTIONS } from './constants';

// ===== ENUMS =====

// Let Go Buddy related enums
export const declutterSituations = pgEnum("declutter_situation", DECLUTTER_SITUATIONS.map(situation => situation.value) as [string, ...string[]]);

export const recommendationActions = pgEnum("recommendation_action", RECOMMENDATION_ACTIONS.map(action => action.value) as [string, ...string[]]);

export const environmentalImpactLevels = pgEnum("environmental_impact_level", ENVIRONMENTAL_IMPACT_LEVELS.map(level => level.value) as [string, ...string[]]);
